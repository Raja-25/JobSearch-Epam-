...
const SavedCompanies = () => {
    ...
    const deleteJob = (index) => {
        const updatedCompanies = [...savedCompanies];
        updatedCompanies.splice(index, 1);
        setSavedCompanies(updatedCompanies);
        localStorage.setItem('savedJobs', JSON.stringify(updatedCompanies));
    ...
    return (
        <div className="container">
            ...
            {savedCompanies.length > 0 ? (
                        ...
                        <tbody>
                            {savedCompanies.map((company, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{company.datePosted}</td>
                                    <td>{company.employmentType}</td>
                                    <td>
                                        <ul className="list-group list-group-flush">
                                            {company.jobProviders.map((provider, idx) => (
                                    ...
                                    <td>
                                        <button className="bin-button" onClick={() => deleteJob(index)}>
                                            <svg
                ...
                <p>No companies saved yet. <Link to="/">Search for companies</Link></p>
            )}
        </div>
    );
}
